...
<div class="wrapper">
 ...
 <div class="col-5" style="margin: 0 auto">
  ...
  <form:form modelAttribute="employeeForm" method="post" action="${saveURL }" cssClass="form" name="myForm">
   <form:hidden path="employeeId"/>
	...
	<form:hidden path="isManager" id="isManager" value="N"/>
   	<table>
   		...
   		<td><form:input type="text" path="firstName" cssClass="form-control" id="firstNameTmp" pattern="[A-ZŹŻŁ]{1}[a-ząęółżść]{2,}" title="Podaj imię pracownika" required="required" onchange="myFullName()"/></td>
   		</tr>  		
   		
   		<tr>
   		<td><label>Nazwisko:</label></td>
   		<td><form:input path="lastName" cssClass="form-control" id="lastNameTmp" pattern="[A-ZŹŻŁĆÓŚĆ]{1}[a-ząęółżść]{2,}" title="Podaj nazwisko pracownika" required="required" onchange="myFullName()"/></td>
   		</tr>
   		
   		<tr>
   		<td><label>Email:</label></td>
   		<td><form:input type="email" path="emailAddress" cssClass="form-control" id="emailAddress" title="Podaj adres email pracownika" required="required"/></td>
   		</tr>
   		...
   		<td><form:select  path="accountType" cssClass="form-control" id="accountType" title="Wybierz typ konta pracowniczego" onchange="isMan()">
	     ...
	     <div class="form-group " style="margin:auto">
	       <form:input path="validFrom"  class="form-control" id="validFrom" name="validFrom"  type="date" title="Podaj datę aktywacji konta" required="required" />
	     </div>
   		</td>
   		</tr>
   		...
   		<td>
	     <div  class="form-group " style="margin:auto">
	       <form:input path="validUntil"  class="form-control" id="validUntil" name="validUntil"  type="date" title="Podaj datę ważności konta" required="required" />
	     </div>
   		</td>
   		</tr>
		...
		<form:option value="zachodnio-pomorskie">zachodnio-pomorskie</form:option>
   		...
   		<td>
   		<div  class="form-group" style="margin:auto">
   		<form:input path="birthDay" class="form-control" type="date" id="birthDay" name="date" />
   		</div>
   		</td>
   		</tr>
	...
	<c:choose>
   		<c:when test="${employeeForm.firstName eq null}">
   			<tr>
	   		<td><button onclick="document.forms['myForm'].reset(); return false;" class="btn btn-primary">Wyczyść dane</button></td>
	   		<td><button onclick="document.forms['myForm'].submit(); return false;" class="btn btn-primary">Dodaj pracownika</button></td>
	   		</tr>
   		</c:when>
   		<c:otherwise>
   			<tr>
	   		<td><button onclick="location.href='/employee/list'"  class="btn btn-primary">Anuluj</button></td>
 ...
 <!-- jQuery CDN -->
		...
		<script>
			function myFullName() {
			    var firstName=document.getElementById("firstNameTmp").value;
			    var lastName=document.getElementById("lastNameTmp").value;
